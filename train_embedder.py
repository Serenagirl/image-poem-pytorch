...
from torch.nn import DataParallel # 多GPU运行
...
class PoemImageEmbedTrainer():
    def __init__(self, train_data, test_data, sentiment_model, batchsize, load_model, device):
        ...
        self.train_transform = transforms.Compose([
            ...
            transforms.RandomHorizontalFlip(),# 依据概率p对PIL图片进行垂直翻转，默认值为0.5
            transforms.ToTensor()
        ])
        ...
        self.train_set = PoemImageEmbedDataset(self.train_data, img_dir,
                                               tokenizer=self.tokenizer, max_seq_len=100,#max_seq_len是最大序列长度，官方为512
                                               transform=self.train_transform)
        self.train_loader = DataLoader(self.train_set, batch_size=batchsize, shuffle=True, num_workers=0)

        self.test_set = PoemImageEmbedDataset(self.test_data, img_dir,
                                              tokenizer=self.tokenizer, max_seq_len=100,
                                              transform=self.test_transform)
        self.test_loader = DataLoader(self.test_set, batch_size=batchsize, num_workers=0)# num_workers是线程数
...
def main():
    ...
    argparser.add_argument('-e', '--num_epoch', type=int, default=10) # 针对所有样本的一次迭代
    ...
    sentiment_model = 'saved_model/sentiment.pth'
